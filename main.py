...
#anigame lottery
@tasks.loop(hours=12 , seconds=30)
async def anigameLotteryLoop():
    await client.get_channel(int(baseChannelID)).send(f'{baseChannelAnigamePrefix}tdaily')
...
#izzi lottery
@tasks.loop(minutes=1 , seconds=1)
async def izziLotteryLoop():
    await client.get_channel(int(baseChannelID)).send(f'{baseChannelIzziPrefix}tcookie kyojuro')

#izzi hourly
@tasks.loop(hours=1 , seconds=30)
async def izziHourlyLoop():
    await client.get_channel(int(baseChannelID)).send(f'{baseChannelIzziPrefix}yui cookie me')

#izzi bt all
@tasks.loop(hours=24 , seconds=30)
async def izziBTALLLoop():
    await client.get_channel(int(baseChannelID)).send(f'{baseChannelIzziPrefix}tquest claim')
